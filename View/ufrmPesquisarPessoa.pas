...
uses
  ...
  ZAbstractDataset, ZDataset, cxRadioGroup, cxDropDownEdit, dxmdaset, DAO.Pesquisa;

type
  TfrmPesquisarPessoa = class(TForm)
  ...
  private
    ...
    procedure BuildSQLString();public
    { Public declarations }
...
var
  frmPesquisarPessoa: TfrmPesquisarPessoa;
  sSQL : String;
  pesquisa : TPesquisa;
implementation
...
uses uGlobais, clUtil, FireDAC.Comp.Client, udm;

procedure TfrmPesquisarPessoa.actFecharExecute(Sender: TObject);
begin
  ModalResult := mrClose;
end;

procedure TfrmPesquisarPessoa.actLocalizarExecute(Sender: TObject);
begin
  InstantSearch();
end;

procedure TfrmPesquisarPessoa.actSelecionarExecute(Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TfrmPesquisarPessoa.edtTextoPropertiesValidate(Sender: TObject; var DisplayValue: Variant; var ErrorText: TCaption;
  var Error: Boolean);
begin
  actLocalizarExecute(Self);
end;

procedure TfrmPesquisarPessoa.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  lListaCampos.Free;
end;

procedure TfrmPesquisarPessoa.FormCreate(Sender: TObject);
begin
  lListaCampos := TStringList.Create();
end;

procedure TfrmPesquisarPessoa.FormShow(Sender: TObject);
begin
  edtTexto.SetFocus;
end;

procedure TfrmPesquisarPessoa.grdPesquisaEnter(Sender: TObject);
begin
...
procedure TfrmPesquisarPessoa.grdPesquisaExit(Sender: TObject);
begin
  cxButton2.Default := False;
end;

procedure TfrmPesquisarPessoa.InstantSearch();
var
...
procedure TfrmPesquisarPessoa.tvPesquisaCellDblClick(Sender: TcxCustomGridTableView; ACellViewInfo: TcxGridTableDataCellViewInfo;
  AButton: TMouseButton; AShift: TShiftState; var AHandled: Boolean);
begin
  actSelecionarExecute(Sender);
end;

procedure TfrmPesquisarPessoa.CaptionColumn();
var
  i : Integer;
begin
...
procedure TfrmPesquisarPessoa.BuildSQLString;
var
  i : integer;
  x : Integer;
begin
