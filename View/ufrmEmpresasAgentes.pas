...
uses
  ...
  clEntrega, clCentroCusto, cxImageComboBox,
...
var
  frmEmpresasAgentes: TfrmEmpresasAgentes;
  agente: TAgente;
  estado: TEstados;
  banco: TBancos;
...
procedure TfrmEmpresasAgentes.actCadastroApoioExecute(Sender: TObject);
begin
  if cxBanco.IsFocused then
  begin
    ListaBancos;
  end;
  if cxCentroCusto.IsFocused then
  begin
    ListaCentroCusto;
  end;
...
procedure TfrmEmpresasAgentes.actCadastroCancelarExecute(Sender: TObject);
begin
  agente.Operacao := 'X';
...
procedure TfrmEmpresasAgentes.actCadastroEditarExecute(Sender: TObject);
begin
  agente.Operacao := 'U';
  Modo;
...
procedure TfrmEmpresasAgentes.actCadastroExcluirEnderecoExecute
  (Sender: TObject);
begin
  ExcluirEndereco;
end;

procedure TfrmEmpresasAgentes.actCadastroExcluirExecute(Sender: TObject);
begin
  ExcluirAgente;
end;

procedure TfrmEmpresasAgentes.actCadastroIncluirExecute(Sender: TObject);
begin
  agente.Operacao := 'I';
  TUtil.LimparFields(Self);
  tbEnderecos.Close;
...
procedure TfrmEmpresasAgentes.actCadastroLocalizarExecute(Sender: TObject);
begin
  Localizar;
end;

procedure TfrmEmpresasAgentes.actCadastroSairExecute(Sender: TObject);
begin
  Close;
end;

procedure TfrmEmpresasAgentes.actCadastroSalvarExecute(Sender: TObject);
begin
  SalvaDados;
end;

procedure TfrmEmpresasAgentes.cxBancoPropertiesChange(Sender: TObject);
begin
  if Pos(agente.Operacao, 'IU') = 0 then
  begin
    Exit;
  end;
  cxNomeBanco.Clear;
...
procedure TfrmEmpresasAgentes.cxBancoPropertiesValidate(Sender: TObject;
  var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
begin
  if Pos(agente.Operacao, 'IU') = 0 then
  begin
...
procedure TfrmEmpresasAgentes.cxCentroCustoPropertiesChange(Sender: TObject);
begin
  if Pos(agente.Operacao, 'IU') = 0 then
  begin
    Exit;
...
procedure TfrmEmpresasAgentes.cxCentroCustoPropertiesValidate(Sender: TObject;
  var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
begin
  if Pos(agente.Operacao, 'IU') = 0 then
  begin
...
procedure TfrmEmpresasAgentes.cxTipoEnderecoPropertiesChange(Sender: TObject);
begin
  if Pos(agente.Operacao, 'IU') = 0 then
  begin
    Exit;
...
procedure TfrmEmpresasAgentes.FormClose(Sender: TObject;
...
begin
  ...
  tbContatos.Close;
  Action := caFree;
  frmEmpresasAgentes := Nil;
end;

procedure TfrmEmpresasAgentes.FormKeyPress(Sender: TObject; var Key: Char);
begin
  If Key = #13 then
  begin
...
procedure TfrmEmpresasAgentes.FormShow(Sender: TObject);
begin
...
procedure TfrmEmpresasAgentes.Modo;
begin
  ...
  else if agente.Operacao = 'S' then
  begin
  ...
  else if agente.Operacao = 'X' then
  begin
    cxGroupBox1.Enabled := False;
    cxGroupBox2.Enabled := False;
    cxGroupBox3.Enabled := False;
...
procedure TfrmEmpresasAgentes.PopulaContatos;
begin
  tbContatos.Close;
  tbContatos.Open;
  tbContatos.IsLoading := True;
  if agente.Contato.getObject(agente.Codigo, 'CODIGO') then
  begin
...
procedure TfrmEmpresasAgentes.PopulaEnderecos;
begin
  tbEnderecos.Close;
  tbEnderecos.Open;
  tbEnderecos.IsLoading := True;
  if agente.Endereco.getObject(agente.Codigo, 'CODIGO') then
  begin
    while not(dm.qryGetObject.Eof) do
    begin
      tbEnderecos.Insert;
...
procedure TfrmEmpresasAgentes.SetupDados;
begin
...
procedure TfrmEmpresasAgentes.LimpaEndereco;
begin
  cxCEP.Clear;
  cxLogradouro.Clear;
  cxNumero.Clear;
...
procedure TfrmEmpresasAgentes.ListaBancos;
var
  sId: String;
begin
  if Pos(agente.Operacao, 'IU') = 0 then
...
procedure TfrmEmpresasAgentes.ExcluirEndereco;
var
  sTipo: String;
begin
  sTipo := '';
...
procedure TfrmEmpresasAgentes.Localizar;
begin
  with dm.qryPesquisa do
  begin
    Close;
    SQL.Clear;
    SQL.Text :=
      'SELECT COD_AGENTE AS "Código", NUM_CNPJ AS "CPF/CNPJ", DES_RAZAO_SOCIAL AS "Nome/Razão Social", '
  ...
  frmLocalizar.cboCamposDisplay.Items.Add
    ('Nº. Ins. Estadual com Subst. Tributária');
  ...
  begin
    if agente.getObject(dm.qryPesquisa.Fields[0].AsString, 'CODIGO') then
    begin
      TUtil.LimparFields(Self);
      SetupDados;
      PopulaContatos;
...
procedure TfrmEmpresasAgentes.ExcluirAgente;
begin
  if agente.Operacao <> 'S' then
  begin
    Exit;
  end;
  if abastecimento.getObject(agente.Codigo, 'ENTREGADOR') then
  begin
    Application.MessageBox
      ('Existem Abastecimentos Registrados para este Agente! Exclusão Cancelada.',
      'Excluir Agente', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  if entregador.getObject(agente.Codigo, 'AGENTE') then
  begin
    Application.MessageBox
      ('Existem Entregadores Registrados para este Agente! Exclusão Cancelada.',
      'Excluir Agente', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  if extravio.getObject(agente.Codigo, 'AGENTE') then
  begin
    Application.MessageBox
      ('Existem Extravios Registrados para este Agente! Exclusão Cancelada.',
      'Excluir Agente', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  if lancamento.getObject(agente.Codigo, 'ENTREGADOR') then
  begin
    Application.MessageBox
      ('Existem Lançamentos de Débitos/Créditos Registrados para este Agente! Exclusão Cancelada.',
      'Excluir Agente', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  if os.getObject(agente.Codigo, 'ENTREGADOR') then
  begin
    Application.MessageBox
      ('Existem Ordens de Serviço Registradas para este Agente! Exclusão Cancelada.',
      'Excluir Agente', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  if planilha.getObject(agente.Codigo, 'AGENTE') then
  begin
    Application.MessageBox
      ('Existem Créditos Registrados para este Agente! Exclusão Cancelada.',
      'Excluir Agente', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  if restricao.getObject(agente.Codigo, 'AGENTE') then
  begin
    Application.MessageBox
      ('Existem Restrições Registradas para este Agente! Exclusão Cancelada.',
      'Excluir Agente', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  if veiculo.getObject(agente.Codigo, 'ENTREGADOR') then
  begin
    Application.MessageBox
      ('Existem Veículos Registrados para este Agente! Exclusão Cancelada.',
      'Excluir Agente', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  if entrega.getObject(agente.Codigo, 'AGENTE') then
  begin
    Application.MessageBox
      ('Existem Entregas Registrados para este Agente! Exclusão Cancelada.',
      'Excluir Agente', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;

  if Application.MessageBox('Confirma a Exclusão deste Agente?',
    'Excluir Agente', MB_YESNO + MB_ICONQUESTION) = IDNO then
  begin
    Exit;
  end;
  ...
  if not(agente.Contato.Delete('CODIGO')) then
  begin
    Application.MessageBox
      ('Erro ao excluir os contatos deste Agente! Exclusão Cancelada.',
      'Excluir Agente', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  if not(agente.Endereco.Delete('CODIGO')) then
  begin
    Application.MessageBox
      ('Erro ao excluir os Endereços deste Agente! Exclusão Cancelada.',
      'Excluir Agente', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  if not(verbacep.Delete('AGENTE')) then
  begin
    Application.MessageBox
      ('Erro ao excluir as Verbas por CEP deste Agente! Exclusão Cancelada.',
      'Excluir Agente', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  if not(agente.Delete('CODIGO')) then
  begin
    Application.MessageBox('Erro ao excluir este Agente!', 'Excluir Agente',
      MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  Application.MessageBox('Exclusão concluída!', 'Excluir Agente',
...
procedure TfrmEmpresasAgentes.SetupClass;
begin
  ...
  if not(TUtil.Empty(cxCNH.Text)) then
  begin
...
begin
  ...
  begin
    ...
    begin
      Application.MessageBox('Erro ao incluir o Agente!', 'Salvar Dados',
        MB_OK + MB_ICONERROR);
      Exit;
    end;
  end
  ...
  begin
    ...
    begin
      Application.MessageBox('Erro ao alterar o Agente!', 'Salvar Dados',
        MB_OK + MB_ICONERROR);
      Exit;
    end;
  end;
...
procedure TfrmEmpresasAgentes.SalvaEnderecos;
begin
  if not(tbEnderecos.Active) then
  begin
    Exit;
  ...
  if not(agente.Endereco.Delete('CODIGO')) then
  begin
    Application.MessageBox('Erro ao Atualizar os Endereços o Agente (EX) !',
  ...
  begin
    ...
    if not(agente.Endereco.Insert()) then
    begin
      Application.MessageBox('Erro ao Atualizar os Endereços o Agente (INS) !',
        'Salvar Dados', MB_OK + MB_ICONERROR);
      Break;
      Exit;
    end;
...
procedure TfrmEmpresasAgentes.SalvaContatos;
begin
  ...
  if not(agente.Contato.Delete('CODIGO')) then
  begin
    Application.MessageBox('Erro ao Atualizar os Contatos o Agente (EX) !',
      'Salvar Dados', MB_OK + MB_ICONERROR);
    Exit;
  end;
  tbContatos.First;
  ...
  begin
    ...
    if not(agente.Contato.Insert()) then
    begin
      Application.MessageBox('Erro ao Atualizar os Endereços o Agente (INS) !',
        'Salvar Dados', MB_OK + MB_ICONERROR);
      Break;
...
function TfrmEmpresasAgentes.SalvaTabelaEndereco(): Boolean;
var
  bFlag: Boolean;
begin
  bFlag := False;
...
procedure TfrmEmpresasAgentes.PopulaEstados;
begin
...
procedure TfrmEmpresasAgentes.ListaCentroCusto;
var
  sId: String;
begin
  if Pos(agente.Operacao, 'IU') = 0 then
...
procedure TfrmEmpresasAgentes.PopulaGrupos;
begin
  ...
  begin
    ...
    while not(Eof) do
    begin
      cxGrupoVerba.Properties.Items.Add(dm.qryPesquisa.FieldByName('GRUPO')
        .AsString);
      Next;
    end;
    Close;
