...
uses
  IdFTP, IdFTPCommon,System.Classes, System.SysUtils, FMX.Dialogs;
...
type
  ...
  public
    ...
    property Pasta: String read getFolder write setFolder;
...
begin
  ...
  try
    try
      ...
      ftp.List;
      ftp.ChangeDir(Self.Pasta + '/' + sDir); // Definir a pasta no servidor
...
begin
  ...
  try
    try
      ...
      if not bExiste then
      begin
        ftp.MakeDir(Self.Pasta + '/' + sDir);
      end;
      ftp.ChangeDir(Self.Pasta + '/' + sDir); // Definir a pasta no servidor
      ftp.Put(ftpUpStream, Self.Pasta + '/' + sDir + '/' + Self.Destino, False);
