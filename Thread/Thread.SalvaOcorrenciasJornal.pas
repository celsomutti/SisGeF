unit Thread.SalvaOcorrenciasJornal;
...
type
  Thread_SalvaOcorrenciasJornal = class(TThread)
  private
...
{ 
  ...
  and UpdateCaption could look like,
    ...
    procedure Thread_SalvaOcorrenciasJornal.UpdateCaption;
    begin
...
{ Thread_SalvaOcorrenciasJornal }

uses View.OcorrenciasJornal, View.ResultatoProcesso;

procedure Thread_SalvaOcorrenciasJornal.IniciaProcesso;
begin
...
procedure Thread_SalvaOcorrenciasJornal.Execute;
var
...
begin
  ...
  try
    ...
    iPos := 0;
    FdPos := 0;
    if not view_OcorrenciasJornal.tbPlanilha.IsEmpty then
      view_OcorrenciasJornal.tbPlanilha.First;
    while not view_OcorrenciasJornal.tbPlanilha.Eof do
    begin
      sMensagem := '';
      if view_OcorrenciasJornal.tbPlanilhaDOM_GRAVAR.AsString = 'S' then
      begin
        SetupClass;
        if view_OcorrenciasJornal.tbPlanilhaNUM_OCORRENCIA.AsInteger > 0 then
        begin
...
procedure Thread_SalvaOcorrenciasJornal.AtualizaProcesso;
begin
...
procedure Thread_SalvaOcorrenciasJornal.TerminaProcesso;
begin
  view_OcorrenciasJornal.pbOcorrencias.Position := 0;
  view_OcorrenciasJornal.pbOcorrencias.Properties.Text := '';
  view_OcorrenciasJornal.pbOcorrencias.Refresh;
  view_OcorrenciasJornal.tbPlanilha.Close;
...
procedure Thread_SalvaOcorrenciasJornal.SetupClass;
begin
