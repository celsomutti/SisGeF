...
uses Forms, StdCtrls, Mask, ExtCtrls, Dialogs, cxControls, cxEdit, cxTextEdit, cxMaskEdit, cxSpinEdit, cxCalendar, cxCheckBox,
     cxButtonEdit, cxDropDownEdit, cxMemo, SysUtils, StrUtils, Windows, FileCtrl, IniFiles, Classes, cxCurrencyEdit, cxCalc,
     ShellAPI, cxTimeEdit, cxImageComboBox, System.DateUtils;
...
type
  ...
  protected
  public
    class function AlinhaE(Texto : String; Tamanho : Integer) : String;
    class function AlinhaC(Texto : String; Tamanho : Integer) : String;
    class function AlinhaD(Texto : String; Tamanho : Integer) : String;
...
class function TUtil.AlinhaE(Texto : String; Tamanho : Integer) : String;
begin
  Texto := Trim(Texto);
...
class function TUtil.AlinhaC(Texto : String; Tamanho : integer) : String;
var
  metade: integer;
begin
  Texto := Trim(Texto);
...
class function TUtil.AlinhaD(Texto : String; Tamanho : Integer) : String;
begin
  Texto := Trim(Texto);
  while length(Texto) < Tamanho do
  begin
...
begin
  ...
  begin
    ...
    begin
      if (Form.Components[i] as TComboBox).Tag <> -1 then
      begin
    ...
    begin
      (Form.Components[i] as TcxComboBox).ItemIndex :=
        (Form.Components[i] as TcxComboBox).Tag;
    end;

    if Form.Components[i] is TcxImageComboBox then
    begin
      (Form.Components[i] as TcxImageComboBox).ItemIndex :=
        (Form.Components[i] as TcxImageComboBox).Tag;
    end;

    if Form.Components[i] is TcxMemo then
      (Form.Components[i] as TcxMemo).Clear;
    ...
    if Form.Components[i] is TcxSpinEdit then
      (Form.Components[i] as TcxSpinEdit).Value := 0;
    ...
    if Form.Components[i] is TcxCalcEdit then
      (Form.Components[i] as TcxCalcEdit).Value := 0;

    if Form.Components[i] is TcxCheckBox then
    begin
      if (Form.Components[i] as TcxCheckBox).Tag <> -1 then
      begin
        (Form.Components[i] as TcxCheckBox).Checked := False;
      end;
    end;

    if Form.Components[i] is TcxDateEdit then
    begin
      if (Form.Components[i] as TcxDateEdit).Tag <> -1 then
      begin
        (Form.Components[i] as TcxDateEdit).Date := 0;
    ...
    begin
      if (Form.Components[i] as TcxTimeEdit).Tag <> -1 then
      begin
        (Form.Components[i] as TcxTimeEdit).Time := 0;
        (Form.Components[i] as TcxTimeEdit).Clear;
      end;
    end;
  end;
end;
...
class function TUtil.CPF(num: String): Boolean;
var
...
class function TUtil.CNPJ(num: String): Boolean;
var
...
class function TUtil.ReplaceStr(const S, Srch, Replace: string): string;
var
  i: Integer;
  Source: string;
begin
...
class function TUtil.SoNumero(kKey: Char): Char;
begin
...
class function TUtil.Sistema(sInfo: String): String;
Var
  Sizever: Integer;
  D: Dword;
  T: UINT;
...
class function TUtil.LeIni(sFile: String; sSecao: String;
...
class function TUtil.GravaIni(sFile: String; sSecao: String; sChave: String;
...
class function TUtil.CriarIni(sFile: string): Boolean;
var
  Ini: TIniFile;
  sMess: String;
begin
...
class function TUtil.FormataCPF(const CPF: String): String;
var
  i: Integer;
begin
  Result := '';
...
class function TUtil.FormataCNPJ(const CNPJ: String): String;
var
  i: Integer;
begin
  Result := '';
...
class function TUtil.ValidaEMail(const EMailIn: PChar): Boolean;
const
...
class function TUtil.NumeroDeLinhasTXT(lcPath: String): Integer;
Var
  aList: TStringList;
Begin
  Result := 0;
...
class function TUtil.GetBuildInfo(prog: String): String;
var
  VerInfoSize: Dword;
  VerInfo: Pointer;
  VerValueSize: Dword;
...
class function TUtil.DSiFileSize(const fileName: string): int64;
var
  fHandle: Dword;
begin
...
class procedure TUtil.Atualiza_Versao_Aplicacao;
var
...
class function TUtil.ENumero(sValor: String): Boolean;
var
  i, iRet: Integer;
  D: Double;
begin
...
class function TUtil.DataOk(Dt: String): Boolean;
var
  Dia, Mes: Byte;
  Ano: Integer;
...
class Function TUtil.VersaoExe(): String;
type
...
class function TUtil.GetStringValue(AHandle: HMODULE; ATexto: String): String;
var
...
class function TUtil.AsciiToInt(Caracter: Char): Integer;
var
  i: Integer;
begin
  i := 32;
...
class Function TUtil.Criptografa(texto:string;chave:integer):String;
var
  cont:integer;
  retorno:string;
begin
...
class Function TUtil.DesCriptografa(texto:string;chave:integer):String;
var
  cont:integer;
  retorno:string;
begin
